...
/// 下慢」的 NAS 盘直接挡在盘点与选择器之外(用户「Z 盘采集不到」的根因之一)。**默认 10s,
/// 到点连不上就跳过该根、继续盘其余的**(用户拍板:NAS 搜不到就不盘,最多等 10 秒直接继续,
/// 绝不为死 NAS 干等),可经 `POLARIS_SCAN_PROBE_SECS` 调。注:这只是「要不要尝试这个根」
/// 的预检,真扫起来还有每目录看门狗兜底,预检放行的半死 NAS 也拖不死盘点。
